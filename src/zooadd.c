...
void modpath(char *);
void opts_add(char *, int *, int *, int *, int *, int *, int *, int *, int *, int *, int *, int *, int *, int *, int *);
...
#define OLD_ZOO 2
	...
	if (zoo_status == NEW_ZOO) {	 /* newly-created archive */
		...
		*latest_name = '\0';
		...
		/*
		 * Skip existing files but add them to a list.  The variable
		 * last_old gets the tail of the old chain of directory entries
		 */
		skip_files(zoo_file, &latest_date, &latest_time, &delcount, latest_name, &last_old);
	}
 ...
 again:			 /* loop back if filename was same as archive name or its backup */
			...
			if (this_path) {
				...
				/*
				 * if moving files, add to list for later deletion;
				 * if list overflows, terminate addition loop and
		...
		if (this_path == NULL) {
			/* in case stdin was read, ensure flist is NULL-terminated */
			flist[fptr] = NULL;
			break;
		}
...
#ifndef PORTABLE
		/* 
		 * Test to see if this is a Z format file.  We assume the file is Z
		 * format if (a) tag is correct and (b) type is 1 and (c) embedded
		 * filename is not longer than FNAMESIZE.
		 */
		if (z_fmt) {
			ssize_t len;
		...
		if (z_fmt) {
			...
			/*
			 * Store long filename into direntry.lfname iff it is
			 * different from MSDOS filename.  Also store directory
			 * name if need_dir is true.  Moved out of zooadd() so
			 * zooadd() doesn't get too big for optimization.
			 */
...
/* define DBG_INLIST for debugging by printing values returned by inlist() */
		...
		if (!update && (!new || !INLIST) || update && (INLIST && RECENT || !INLIST && new))
		...
		zooseek(zoo_file, direntry.offset - SIZ_FLDR, 0);
		zoowrite(zoo_file, file_leader, SIZ_FLDR);

#ifdef PORTABLE
		prterror('m', "%-*s -- ", longest, this_path);
#else
		if (z_fmt)
			prterror('m', "%-12s <== %-*s -- ", direntry.fname, longest, this_path);
		else
			prterror('m', "%-*s -- ", longest, this_path);
#endif					 /* PORTABLE */
...
#include "UNBUF_IO not currently supported"
		...
		if (status != 0) {
			...
			zooseek(this_file, 0L, 2); /*DEBUG VMS */
			/* seek to EOF */
		...
		if (success) {
...
#endif
			if (!z_fmt)
				prterror('M', " (%2d%%) ", cfactor(direntry.org_size,
								   direntry.size_now));
			...
			if (prev_pos > 0) {
				...
				if ((zoo_header.vdata & VFL_ON) && (high_vflag & VFL_ON)) {
					...
					if (high_pos != prev_pos || this_version_no == 1) {
						...
						long save_pos = zootell(zoo_file);

						zooseek(zoo_file, high_pos, 0);	/*DEBUG*/
				...
				if (delold) {	/* deleting old file */
					...
					readdir(&dir2entry, zoo_file, 1);
					if (dir2entry.cmt_size != 0) {
						/* propagate latest comment */
						oldcmtpos = dir2entry.comment;
						oldcmtsiz = dir2entry.cmt_size;
					}
					dir2entry.deleted = 1;
			...
			/*
			 * if user requested comments, any previous comment in a Z
			 * format file may now be manually overwritten
			 */
			...
			if (z_fmt) {
				/* moved out to shorten code & allow optimizer to work */
	...
	if (!addcount) {
		...
		if (delcount && pack) {
			/* pack if user asked and found deleted entries */
			prterror('M', "-----\nPacking...");
			zoopack(zoo_path, "PP");
			prterror('M', "done\n");
		}
