...
/***********
 * memerr() Give error message on memory error and abort
 */
void memerr(i)
unsigned int i;
{
...
/***********
 * emalloc() allocates memory like malloc() does (and it calls malloc).
 * However, it automatically calls the error function memerr() if memory
 * couldn't be allocated.  It also assumes that memory will never be
 * freed and conserves it by allocating memory in large chunks and then
 * partitioning it out with no administrative overhead.  This avoids
 * many problems due to various bugs in various implementations of
 * malloc, and also avoids a very high amount of malloc overhead for
 * repeated allocations to store numerous filenames etc.  The down side
 * is that we can't use free().
 *
 * WARNING: No alignment of allocated memory is attempted, so memory
 *          allocated through emalloc should be used only for storing
 *          strings.  For allocating memory for other data types use
 *          ealloc().
 */
...
/***********
 * ealloc() is just a wrapper around malloc(), which causes memerr() to
 * be called if memory cannot be allocated.  All memory requests are
 * passed on to malloc.  Allocated memory can later be freed.
 */
VOIDPTR ealloc(size)
unsigned int size;
{
...
/***********
 * putstr() - prints a string to standard output.  If the received
 ...
 * supplied the filename.  It must return 1 if the file exists and 0 if
 * it does not.
 * 
 ...
 *
 * Because of the delay between the time existence is checked and the
 * time Zoo creates a files, a race condition exists.  It would be
 * better to use open() with the O_EXCL flag but that will not work for
 * many systems.
 */
int exists(fname)
char *fname;
...
/***********
 * newcat() allocates enough space to concatenate two strings then
 * returns a pointer to the concatenated result 
 */
char *newcat(r, s)
char *r, *s;
{
...
/***********
 * addext() - if no extension in filename add supplied extension
 */
...
#ifdef VER_CH
/* remove any trailing extension field */
...
/***********
 * samefile() compares two filenames to see if they are the same file.
 * Just strcmp() or str_icmp() could have been used, except that if the
 * filenames have trailing version fields, we want to compare those
 * always equal.  samefile() is called by routines that want to avoid
 * adding an archive to itself.
 */
int samefile(f1, f2)
char *f1;
char *f2;
...
/***********
 * tzadj() accepts a directory entry and adjusts its timestamp to
 * reflect its timezone.  Uses function mstime() from mstime.i and
 * mstonix() from nixtime.i.
*/
void tzadj(direntry)
...
/***********
 * add_version() adds a version suffix to a filename, given the
